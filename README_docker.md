...
docker run -it cmake_template:latest
```
...
```bash
docker build --tag=cmake_template:latest --build-arg GCC_VER=10 --build-arg LLVM_VER=11 .
```
...
```bash
docker build --tag=cmake_template:latest --build-arg USE_CLANG=1 .
```

You will be logged in as root, so you will see the `#` symbol as your prompt.
You will be in a directory that contains a copy of `cmake_template`;
...
cmake_template:latest
```

You can configure and build [as directed above](#build) using these commands:

```bash
/cmake_template# mkdir build
/cmake_template# cmake -S . -B ./build
/cmake_template# cmake --build ./build
```
...
```bash
/cmake_template# mkdir build
/cmake_template# CC=clang CXX=clang++ cmake -S . -B ./build
/cmake_template# cmake --build ./build
```
